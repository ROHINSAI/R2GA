...
def _calc_system_utilization(scheduling_list, processor_number, makespan):
    """Fraction of (makespan × #processors) time that is actively used."""
...
def _calc_workload_balance(scheduling_list, processor_number):
    """Max processor finish-time minus min non-zero processor finish-time.
...
def _calc_energy(scheduling_list):
    """Sum of (computation duration * speed factor) across all tasks.
    speed_factor mirrors the pc = [1,1,2,2,3,3] vector used in readfile."""
...
def _combined_fitness(makespan, workload_balance, energy, utilization):
    """Lower is better.  Utilization is subtracted (higher utilization = better)."""
    alpha = 0.1   # workload balance weight
    beta  = 0.01  # energy weight
    gamma = 0.5   # utilization bonus weight (scaled by makespan)
    return makespan + alpha * workload_balance + beta * energy - gamma * utilization * makespan


# ── Scheduler class ───────────────────────────────────────────────────────────

class GeneticScheduler1(Scheduler):
